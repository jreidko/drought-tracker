...
export default function LeaderboardError({
  message = "The MLB Stats API did not respond. Try again in a few minutes.",
  retryHref = "/droughttracker",
}: {
  message?: string;
  retryHref?: string;
}) {
  return (
    <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-4 sm:px-6 sm:py-6">
      <section className="rounded-sm border border-sith/40 bg-sith/5 px-4 py-8 text-center sm:px-6">
        ...
        <p className="mt-2 text-sm text-muted sm:text-base">{message}</p>
        <Link
          href={retryHref}
